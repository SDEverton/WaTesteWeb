...
const PedidosListPage = memo(() => {
  ...
  const [params, mergeParams, loading, data, error, , refresh] = usePaginationObservable(
    params => requestsService.list(params),
      ...
      <Card>
        ...
        <TableWrapper minWidth={500}>
          <Table>
            <TableHead>
              <TableRow>
                <TableCellSortable paginationParams={params} disabled={loading} onChange={mergeParams}>
                  Descrição
                </TableCellSortable>
            ...
            <TableBody>
              <EmptyAndErrorMessages
                ...
                onTryAgain={refresh}
              />
              {results.map(requests => (
                <ListItem key={requests.id} requests={requests} onEdit={handleEdit} onDeleteComplete={refresh} />
              ))}
            </TableBody>
          </Table>
        </TableWrapper>
